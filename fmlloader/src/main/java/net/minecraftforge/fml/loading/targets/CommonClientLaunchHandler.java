...
public abstract class CommonClientLaunchHandler extends CommonLaunchHandler {
    ...
    public LocatedPaths getMinecraftPaths() {
        ...
        var fabriclang = LibraryFinder.findPathForMaven(vers.forgeGroup(), "fabriclanguage", "", "", vers.mcAndForgeVersion());
        var mclang = LibraryFinder.findPathForMaven(vers.forgeGroup(), "mclanguage", "", "", vers.mcAndForgeVersion());

        return new LocatedPaths(mcstream.build().toList(), (a,b) -> true, modstream.build().toList(), List.of(fmlcore, javafmllang, lowcodelang, fabriclang, mclang));
    }
